...
class ConstrainedDecoder:
    ...
    def _advance_state(
        ...
        if state == JsonState.START:
            ...
            current_key += char
            if (
                current_key.startswith('"')
                and current_key.endswith('"')
                and len(current_key) > 1
            ):
            ...
            if escape_next:
                ...
                if string_depth == 0:
                    ...
                    if key in remaining_keys:
                        remaining_keys = [
                            k for k in remaining_keys if k != key
                        ]
                    current_key = ""
                    current_value_raw = ""
                    state = JsonState.COMMA_OR_END
    ...
    def _get_valid_name_tokens(
        ...
        candidates = [
            n for n in function_names if n.startswith(generated_so_far)
        ]
        if not candidates:
            return []
